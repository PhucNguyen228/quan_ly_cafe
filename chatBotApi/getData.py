...
# Thực hiện truy vấn SQL để lấy dữ liệu từ bảng sản phẩm
cursor.execute("SELECT anh_dai_dien, ten_san_pham, gia_ban, gia_khuyen_mai FROM san_phams")
...
# Duyệt qua các kết quả và tạo các mục trong intents
for result in results:
    anh_dai_dien, ten_san_pham, gia_ban, gia_khuyen_mai = result

    # Tạo pattern từ tên sản phẩm
    pattern = r"" + ten_san_pham + r""

    # Tạo response từ thông tin sản phẩm
    response = f"Image: {anh_dai_dien}\nProduct: {ten_san_pham}\nPrice: {gia_ban}\nDiscount Price: {gia_khuyen_mai}\n"
