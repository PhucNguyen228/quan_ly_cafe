                    ...
                    <table class="mb-0 table table-bordered" id="tableBan">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center">Thiết Lập</th>
                                <th class="text-center">Tình Trạng Bàn Hiện Tại</th>
...
@section('js')
    <script>
            ...
            $.ajax({
                ...
                success :   function(res) {
                    ...
                    $.each(res.dulieu, function(key, value) {
                        ...
                        if(value.is_open_oder == 0){
                            var class_button_oder = 'btn-danger';
                            var button_open  = 'Full';
                        }else{
                            var class_button_oder = 'btn-primary';
                            var button_open  = 'Trống';
                        }
                        content_table += '<tr>';
                        content_table += '<th class="text-center" scope="row">' + (key + 1) +'</th>';
                        content_table += '<td> ' + value.ma_ban +' </td>';
                        content_table += '<td class="text-center">';
                        content_table += '<button data-id="'+ value.id +'" class="thietLap btn '+ class_button +'">';
                        content_table +=  text_button;
                        content_table += '</button></td>';
                        content_table += '<td class="text-center">';
                        content_table += '<button data-idtinhtrang="'+ value.id +'" class="doitinhtrang btn '+ class_button_oder +'">';
                        content_table +=  button_open;
                        content_table += '</button></td>';
                        content_table += '<td class="text-center">';
        ...
        $('body').on('click','.thietLap',function(){
            var idBan = $(this).data('id');
            var self = $(this);
            $.ajax({
                url     :     '/admin/ban/thiet-lap/' + idBan,
                type    :     'get',
                success :     function(res) {
                    if(res.trangThai) {
        ...
        $('body').on('click','.doitinhtrang',function(){
            var idBan = $(this).data('idtinhtrang');
            var self = $(this);
            $.ajax({
                url     :     '/admin/ban/tinh-trang/' + idBan,
                type    :     'get',
                success :     function(res) {
                    if(res.trangThai) {
                        toastr.success('Đã đổi trạng thái thành công!');
