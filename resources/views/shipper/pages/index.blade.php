...
@section('content')
    ...
    <div style="margin-left: 85%" class="btn btn-warning"><a href="/shipper/don-nhan" style="color: white;">Đơn hàng đã nhận</a></div>
@endsection
    ...
    <script>
        $(document).ready(function() {
                        ...
                        $.each(res.dataTinhTrang, function(key, value) {
                            if (value.tinh_trang_don_hang == 2) {
                                var doan_muon_hien_thi =
                                    '<div class="btn btn-primary">Nhận hàng</div>';
                            }
                            html += '<tr>';
            ...
            function loadTableRight(id) {
                $.ajax({
                    url: '/shipper/chi-tiet-don-hang/data/' + id,
                    type: 'get',
                    success: function(res) {
                        console.log(res);
                        var content_table = '';
            ...
            $('body').on('click', '.show', function() {
                ...
                $.ajax({
                    url: '/shipper/chi-tiet-don-hang/' + id,
                    type: 'get',
                    success: function(res) {
                        if (res.status) {
                            loadTableRight(id);
            ...
            $('body').on('click', '.tinhtrang', function() {
                ...
                $.ajax({
                    url: '/shipper/nhan-hang/' + id,
                    type: 'get',
                    success: function(res) {
                        if (res.status) {
                            toastr.success("đã nhận hàng!");
                            loadTable();
                        }
                    },
                });
